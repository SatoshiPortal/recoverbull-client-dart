...
class SocialRecoveryPage extends StatefulWidget {
  ...
  @override
  State<SocialRecoveryPage> createState() => SocialRecoveryPageState();
}

class SocialRecoveryPageState extends State<SocialRecoveryPage> {
  ...
  var pemFilename = '';
  var pemFile = '';
  var encryptedKeyFilename = '';
  var encryptedKeyFile = '';
  var encryptedBackupKey = '';
  ...
  void _pickPEM() async {
    ...
    if (result != null && result.files.single.bytes != null) {
      pemFilename = result.files.single.name;
      pemFile = utf8.decode(result.files.single.bytes!);
      setState(() {});
    }
  }

  void _pickEncryptedKey() async {
    final result = await FilePicker.platform.pickFiles(
      type: FileType.custom,
    ...
    if (result != null && result.files.single.bytes != null) {
      encryptedKeyFilename = result.files.single.name;
      encryptedKeyFile = utf8.decode(result.files.single.bytes!);
      setState(() {});
    }
  }
  ...
  void _handleWebSocketMessage(String message) async {
    ...
    switch (eventType) {
      ...
      case "social_recovery":
        final senderPin = data["sender_pin"];
        encryptedBackupKey = data["backup_key"];

        final backupKey =
            await decryptMessage(encryptedBackupKey, Global.pair.privateKey);

        await FileSaver.instance.saveFile(
          name: 'decrypted-backup-key.txt',
          bytes: Uint8List.fromList(utf8.encode(backupKey)),
        );

        show(senderPin,
            "Peer sent you back your backup key encrypted, check your download folder");
        break;
  ...
  Future<void> recoverBackupKey(
    WebSocketChannel channel,
    String myPin,
    String peerPin,
    String peerPubKey,
  ) async {
    final privateKey = pemFile;
    final publicKey = await RSA.convertPrivateKeyToPublicKey(privateKey);
    final pair = KeyPair(publicKey, privateKey);

    // Decrypt the encrypted backup key with my private key
    final decryptedBackupKey =
        await decryptMessage(encryptedKeyFile, pair.privateKey);

    // Encrypt the decrypted backup key with peer public key to communicate it safely
    final encryptedBackupKey =
        await encryptMessage(decryptedBackupKey, peerPubKey);
            ...
            if (isPeered)
              Row(
                ...
                children: [
                  ...
                  if (isPeered && isRecovering == false)
                    ElevatedButton(
                      onPressed: () => setState(() => isRecovering = true),
                      child: const Text('Recover Backup Key'),
                    ),
                  if (isRecovering && hasRecovered == false)
                    ElevatedButton(
                      onPressed: _pickPEM,
                      child: Text(pemFilename.isEmpty
                          ? 'Select your private key'
                          : pemFilename),
                    ),
                  if (isRecovering && hasRecovered == false)
                    ElevatedButton(
                      onPressed: _pickEncryptedKey,
                      child: Text(encryptedKeyFilename.isEmpty
                          ? 'Select encrypted backup key'
                          : encryptedKeyFilename),
                    ),
                  if (isRecovering &&
                      hasRecovered == false &&
                      encryptedKeyFile.isNotEmpty &&
                      pemFile.isNotEmpty)
                    ElevatedButton(
                      onPressed: () async {
                        await recoverBackupKey(
                            channel, _myPin, _peerPin, _peerPubKey);
                      },
                      child: const Text('Send back to peer'),
                    ),
                  if (isRecovering == false)
                    ElevatedButton(
                      onPressed: () {
